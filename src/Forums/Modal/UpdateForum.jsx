...
export default function UpdateForum({
  ...
  useEffect(() => {
    const getForumById = async () => {
      await axios
        .get(`http://localhost:5000/forum/${idUpdateForum}`)
        .then((response) => {
          setNamaForum(response.data.namaForum);
  ...
  const updateForum = async (e) => {
    ...
    try {
      ...
      await axios.patch(
        `http://localhost:5000/forum/${idUpdateForum}`,
        formData,
        {
          headers: {
  ...
  const maxLength = 20; //Batas limit title
        ...
        >
          <div className="justify-end items-end">
            ...
            <button
              ...
              onClick={() => setUpdateForumModal(false)}
            >
          ...
          {/* FORM */}
          <form className="mt-5" onSubmit={updateForum}>
            {/* Input Nama Forum */}
            <div>
